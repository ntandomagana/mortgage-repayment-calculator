...
document.querySelectorAll('.mortgage-type').forEach(input => { // looks for this 'mortgae-type' element and stores it in this variable
 ...
 // when the user clicks this button, the function will be executed
document.getElementById('calculate-btn').addEventListener('click', () => {

    //this retrieves the value of the ID's and changes it to a floating-point number.
...
// removes error from elements with this class
    document.querySelectorAll('.form-flex').forEach(el => {
        el.classList.remove('error');
    })

    //this function checks if user has entered a numeric value, not negative or below 0
    if(isNaN(amount) || amount <= 0) { 
        //this happens if amount is invalid
        document.getElementById('amount-alert').style.display = 'block'
        document.getElementById('mortgage-amount-main').classList.add('error');
        isValid = false;

        // this else statement will remove error messages if the input is valid
    } else {
        document.getElementById('amount-alert').style.display = 'none';
    }
// This code is similar to previous oone, but checks the term. If term number is valid
// then error messages are hidden
    ...
    if (!mortgageType) { // no mortgage type has been selected
    ...
    if (isValid) {
        ...
        calculationsContainer.classList.remove('hide');
...
//clears everything and resets the form
